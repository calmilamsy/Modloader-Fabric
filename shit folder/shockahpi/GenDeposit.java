...
public class GenDeposit {
   ...
   public void gen(Loc startLoc, int maxAmount, int maxTries) {
      if(this.set1stOn.contains(Integer.valueOf(startLoc.getBlock(this.world)))) {
         ...
         while(maxAmount > 0 && maxTries > 0 && !this.check.isEmpty()) {
            ...
            if(this.setOn.contains(Integer.valueOf(curLoc.getBlock(this.world)))) {
   ...
   public int getGoodY(int pX, int pY, int pZ) {
      int off = 0;
      if(this.set1stOn.contains(Integer.valueOf(this.world.getBlockId(pX, pY, pZ)))) {
         return pY;
      } else {
         do {
            ++off;
            if(pY + off <= 127 && this.set1stOn.contains(Integer.valueOf(this.world.getBlockId(pX, pY + off, pZ)))) {
               return pY + off;
            }

            if(pY - off >= 0 && this.set1stOn.contains(Integer.valueOf(this.world.getBlockId(pX, pY - off, pZ)))) {
               return pY - off;
            }
